...
export default async function handler(req, res) {
  ...
  try {
    const url = `https://api.openweathermap.org/data/2.5/forecast?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}&appid=${process.env.OWM_API_KEY}&units=${normUnits(units)}&lang=${normLang(lang)}&cnt=40`;
